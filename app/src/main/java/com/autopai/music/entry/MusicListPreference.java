...
public class MusicListPreference extends ListPreference {
    private CharSequence[] mEntrySummaries;

    public MusicListPreference(Context context) {
        super(context);
        initPreference(context, null);
    }

    public MusicListPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        initPreference(context, attrs);
    }

    public MusicListPreference(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initPreference(context, attrs);
    }

    public MusicListPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initPreference(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.MusicListPreference, 0, 0);
        if (a == null)
            return;

        mEntrySummaries = a
                .getTextArray(R.styleable.MusicListPreference_entrySummaries);
